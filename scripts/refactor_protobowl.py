...
def ingest_file(
    ...
    with input_path.open("r", encoding="utf-8") as handle, closing(connection.cursor()) as cursor:
        for line in handle:
            ...
            user_key, user_payload = normalize_user(obj.get("user"))
            cursor.execute(
                insert_user,
                (
                    user_key,
...
def write_buzzes(
    ...
    with closing(connection.cursor()) as cursor:
        for user_key, user_payload_json, buzz_count, payload_json in cursor.execute(
            ...
            if user_key != current_user_key:
                ...
                grouped_users.append(current_group)
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(grouped_users, handle, indent=2, sort_keys=True)
        handle.write("\n")

    return total_buzzes, rate_ratios, time_ratios
...
def write_histogram_svg(
    ...
    rate_hist = build_histogram(rate_ratios, bins, upper_bound)
    time_hist = build_histogram(time_ratios, bins, upper_bound)
    all_counts = [item["count"] for item in rate_hist + time_hist]
    ...
    top_margin = 60
    gap = 70
    first_top = top_margin
    second_top = first_top + panel_height + gap
    ...
    def panel_svg(
        ...
        if not histogram:
            return (
                f'<text x="{margin_left}" y="{top + 40}" font-size="18" '
                f'font-family="sans-serif">{title}: no data</text>'
            )

        bin_width = chart_width / len(histogram)
        elements = [
            f'<text x="{margin_left}" y="{top - 18}" font-size="18" font-family="sans-serif">{title}</text>',
            f'<line x1="{margin_left}" y1="{top + panel_height}" x2="{margin_left + chart_width}" y2="{top + panel_height}" stroke="#333" stroke-width="1"/>',
            f'<line x1="{margin_left}" y1="{top}" x2="{margin_left}" y2="{top + panel_height}" stroke="#333" stroke-width="1"/>',
        ]

        for tick in range(5):
            y = top + panel_height - (panel_height * tick / 4.0)
        ...
        for index, bucket in enumerate(histogram):
            ...
            elements.append(
                f'<rect x="{x:.1f}" y="{y:.1f}" width="{max(bin_width - 4, 1):.1f}" height="{bar_height:.1f}" fill="{color}" opacity="0.8"/>'
            )

        label_step = max(1, len(histogram) // 5)
        for index, bucket in enumerate(histogram):
            if index % label_step != 0 and index != len(histogram) - 1:
                continue
            x = margin_left + index * bin_width
            elements.append(
                f'<text x="{x:.1f}" y="{top + panel_height + 20}" font-size="11" font-family="sans-serif">{bucket["start"]:.2f}</text>'
            )
  ...
  <text x="{margin_left}" y="50" font-size="13" fill="#555" font-family="sans-serif">Top: rate ratio = (elapsed / playback_rate) / len(question_text). Bottom: time ratio = elapsed / (elapsed + remaining). X-axis clipped to 1.25.</text>
  {panel_svg("Rate ratio", first_top, rate_hist, "#CC412F")}
  {panel_svg("Time ratio", second_top, time_hist, "#4C78A8")}
</svg>
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Normalize a Protobowl buzz log into question and buzz JSON outputs."
    )
    ...
    parser.add_argument(
        "--buzzes",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "protobowl-buzzes-by-user.json",
        help="Output path for normalized buzz records grouped by user",
    )
    parser.add_argument(
        "--histogram",
        type=Path,
...
def main() -> int:
    ...
    args.buzzes.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        with sqlite3.connect(sqlite_path) as connection:
            ...
            buzz_count, rate_ratios, time_ratios = write_buzzes(
                connection,
                args.buzzes,
                args.minimum_user_buzzes,
            )

        write_histogram_svg(args.histogram, rate_ratios, time_ratios)

        print(
            (
                f"Wrote {question_count:,} questions to {args.questions}, "
                f"{buzz_count:,} buzzes to {args.buzzes}, and histogram to {args.histogram} "
